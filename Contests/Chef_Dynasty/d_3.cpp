...
int main(){
    ...
    while(q--){
        cin >> a >> b;
        int total_cost = 0;
        int same_path_a_flag = 0, same_path_b_flag = 0;
        if (a != b){
            map <int, int> path;
            vector <int> cost_a;
            vector <int> cost_b;
            ...
            while (parent_ptr_a != nullptr){
                ...
                if (parent_key == b){same_path_a_flag = 1; break;}
                child_node_a = *parent_ptr_a;
                parent_ptr_a = parent_ptr_a->parent;
            }

            if (same_path_a_flag == 0){
                Node child_node_b = node_list[b - 1];
                Node* parent_ptr_b = child_node_b.parent;
                while (parent_ptr_b != nullptr && path[parent_ptr_b->key] != 1){
                    ...
                    if (parent_ptr_b->key == a){same_path_b_flag = 1; break;}
                    child_node_b = *parent_ptr_b;
                    parent_ptr_b = parent_ptr_b->parent;
                }

                if (parent_ptr_b != nullptr && same_path_b_flag == 0){
                    path_b.push_back(parent_ptr_b->key);
                    cost_b.push_back(child_node_b.parent_rel);
                }
            ...
            if (same_path_b_flag == 0){
                ...
                for (int i = 0; i < (int)cost_b.size(); i++){
                    if (cost_b[i] > 0){total_cost += cost;}
                }
            }
        }
